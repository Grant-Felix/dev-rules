	...
	factory: (require) => {
		...
		function RuleRow(props) {
			...
			return h(
				...
				h(
					...
					h('span', { className: 'dr_size', title: '这条规则给每次对话增加的大致上下文量' }, '约 ' + String(ruleChars(rule)) + ' 字'),
					h(
						'span',
		...
		function RuleList(props) {
			...
			return h(
				...
				h(
					'div',
					{ className: 'dr_row' },
					h(Button, { onClick: addRule }, '+ 加一条规则'),
		...
		function DevRulesPanel() {
			...
			const save = useCallback(async () => {
				if (doc === null) return;
				const blank = doc.projects.find((project) => String(project.path).trim() === '');
				if (blank !== undefined) {
					setTab('projects');
					setMessage({ kind: 'error', text: '有一个项目还没填目录，补上或删掉它再保存。' });
					return;
				}
				setBusy(true);
				try {
			...
			return h(
				...
				h(
					...
					{ className: 'dr_body' },
					tab === 'global'
						? h(GlobalTab, { doc, mutate, query, groupFilter })
						: tab === 'projects'
							? h(ProjectsTab, { doc, mutate, query, groupFilter })
...
`;
		...
		exports.__internal = { estimateTokens, mergeDocs, injectedSummary, resultSummary, shouldShowFilters, canShortcutSave };
