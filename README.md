...
发版版本号用**发布日期式**：对外一律 `v<YY>.<M>.<D>.<当日序号>`，例如 `v26.9.21.1`（2026-09-21 当天第 1 个版本），同日第 2 个是 `v26.9.21.2`；年月日**不补零**。清单里的 `version` 只写前三段（`26.9.21`）—— npm 与 Cargo 都硬性要求三段 semver，「当日序号」只进标签。三个平台推同一个 tag。
